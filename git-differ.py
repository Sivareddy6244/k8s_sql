...
CURRENT_PYTHON_RESPONSE_FILE = "current_python_response.txt"
...
def process_directory(directory_path, output_file_path, include_subdirs=False):
    """Processes each file in the directory and generates a review."""
    code_files = []

    for root, _, files in os.walk(directory_path):
        for file in files:
            if file.endswith(('.py', '.sqlx', '.sql')):
...
def main():
    # Process directory to generate reviews
    process_directory(PYTHON_DIR, CURRENT_PYTHON_RESPONSE_FILE, include_subdirs=True)

    # Compare old and new responses to generate unique issues report
    generate_unique_issues_report(OLD_PYTHON_RESPONSE_FILE, CURRENT_PYTHON_RESPONSE_FILE, FINAL_OUTPUT_FILE)
